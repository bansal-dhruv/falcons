...
DRAG_LIST = [2,2,2,7,7,7,15,15,15,20,20,20,20,20,25,25,30,35]
DEGREE_COUNTER = 0
MOUTH_COUNTER = 0
EYE_COUNTER = 0
WINK_COUNTER = 0
    ...
    while True:
        ...
        if INPUT_MODE:
            ...
            if dir == 'right':
                ...
                if tp - 5 >l+DRAG_LIST[continue_drag] :
                    autopy.mouse.smooth_move(l + DRAG_LIST[continue_drag],b)
                # pag.moveRel(DRAG_LIST[continue_drag], 0)
            elif dir == 'left':
                if current_direction != 'left':
                    continue_drag = 0
                    current_direction = 'left'
                if 2 < l-DRAG_LIST[continue_drag] :
                    autopy.mouse.smooth_move(l - DRAG_LIST[continue_drag],b)
                # pag.moveRel(-DRAG_LIST[continue_drag], 0)
            elif dir == 'up':
                ...
                if SCROLL_MODE:
                    ...
                    if 2 < b - DRAG_LIST[continue_drag]:
                        autopy.mouse.smooth_move(l , b - DRAG_LIST[continue_drag])
                ...
                if SCROLL_MODE:
                    ...
                    if btm-5 > b + DRAG_LIST[continue_drag]:
                        autopy.mouse.smooth_move(l, b + DRAG_LIST[continue_drag])
                    # pag.moveRel(0, DRAG_LIST[continue_drag])
            else:
                current_direction = None
